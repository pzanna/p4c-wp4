...
ControlBodyTranslator::ControlBodyTranslator(const WP4Control* control) :
...
void ControlBodyTranslator::compileEmitField(const IR::Expression* expr, cstring field, unsigned alignment, WP4Type* type) {
...
bool ControlBodyTranslator::preorder(const IR::SwitchStatement* statement) {
    ...
    for (auto c : statement->cases) {
        ...
        if (c->label->is<IR::DefaultExpression>()) {
            ...
            cstring name = WP4Object::externalName(act);
            builder->append(name);
        }
        builder->append(":");
        builder->newline();
...
void WP4Control::emitDeclaration(CodeBuilder* builder, const IR::Declaration* decl) {
...
void WP4Control::emit(CodeBuilder* builder) {
    auto hitType = WP4TypeFactory::instance->create(IR::Type_Boolean::get());
    builder->emitIndent();
...
class OutHeaderSize final : public CodeGenInspector {
 ...
 public:
    OutHeaderSize(P4::ReferenceMap* refMap, P4::TypeMap* typeMap, const WP4Program* program):
...
WP4Deparser::WP4Deparser(const WP4Program* program, const IR::ControlBlock* block, const IR::Parameter* parserHeaders) :
        WP4Control(program, block, parserHeaders), packet(nullptr) {}

bool WP4Deparser::build() {
    auto pl = controlBlock->container->type->applyParams;
    if (pl->size() != 3) {
...
void WP4Deparser::emit(CodeBuilder* builder) {
    OutHeaderSize ohs(program->refMap, program->typeMap,static_cast<const WP4Program*>(program));
